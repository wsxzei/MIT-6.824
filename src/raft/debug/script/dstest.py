...
def run_test(test: str, race: bool, timing: bool):
    ...
    # 使用临时文件, 将标准输出重定向到临时文件
    f, path = tempfile.mkstemp()
    start = time.time()

    proc = subprocess.run(test_cmd, stdout=f, stderr=f) # 启动子进程, 执行go test 命令
...
# fmt: off
# eg: dstest 2A -p 40 -n 5000 -o debug/raft_log, dstest='python3.8 dstest.py'
def run_tests(
        tests: List[str],
                ...
                while completed < total:
                    ...
                    for future in done:
                        ...
                        if rc != 0 or archive:
                            ...
                            # 先使用路径为 path 的临时文件存储单个测试的日志, 如果测试失败，输出到指定的目录下
